...
double Trapeze::Square(){
	double value = (1.0 / 4.0) * (b + Get_a()) / (b - Get_a());
	return value * sqrt( (-Get_a() + b + c + d) * (Get_a() - b + c - d) * (Get_a() - b - c + d) );
}

//віртуальний метод --- відповідає за обчислення периметра трапеції
double Trapeze::Perimeter() { return (Get_a() + b + c + d); }
